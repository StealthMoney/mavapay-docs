...
<Note>
  Cache the bank list for 24 hours to reduce API calls.
</Note>

### Step 2: Get the Exchange Rate (Optional)

Use the price ticker endpoint to preview the current rate before creating a quote. The `ask` price reflects the rate for sending Rand using NGN.
...
<Note>
  The rate is returned as the price of **1 Rand in Kobo**. A fresh quote gives you the exact amount; use the ticker for indicative pricing only.
</Note>

### Step 3: Create a Quote

Create a quote to lock in the exchange rate and receive a virtual NGN bank account to fund the transfer.

#### Understanding Payment Currency

The `paymentCurrency` parameter determines what the `amount` field represents:

- **`paymentCurrency: "NGNKOBO"`** → amount is in Naira (you're specifying how much to send)
...
| `exchangeRate` | Rate applied (Kobo per Cent) |
...
<Warning>
  You must send the **exact amount** shown in the quote. Partial payments are not automatically processed.
</Warning>
...
A: Check the API response for current minimum limits.
