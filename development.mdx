...
#### Stablecoins (USDT)
Test USDT deposits and withdrawals on staging:
- Generate deposit addresses on supported networks (`trc20`, `erc20`, `plasma`, `solana`, `polygon`)
- Withdraw USDT from your test wallet to an external address
- Receive the same `payment.received` and `payment.sent` webhooks as production

See the [stablecoins guide](/stablecoins/introduction) for the full deposit and withdraw flow.
